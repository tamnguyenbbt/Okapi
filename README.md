...
* Unlike many free and commercial test automation frameworks in the market, providing good user interfaces, utilities, a compiler, an object repository, a not very smart recording and object spying tool based on traditional html searching methods (id, name, tag name, class, css, xpath, etc.), and an API library then let users to deal with all the real-life tough test automation scripting obstacles such as timing, single-page application automation obstacles, AJAX etc. (that why a lot of organizations after buying them, have to find an experiened and technical strong automation tester and form a large team to build test automation frameworks around them. Sometimes, they have to involve some good developers to build the automation test frameworks around them. Unfortunately, developers have little knowledge and experience on real-life test automation difficulties and quite often they fail or create messy frameworks.), Okapi deals with all these things, letting users to focus on business rules while scripting (there is no need to build automation framework around Okapi).   
...
## Versions
* Version **1.2.16** released on 29/07/2019
